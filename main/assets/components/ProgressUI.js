...
// targetColId : targetCol.id
    ...
    return `
  <div id="progressUi">
    <form name="progressForm" action="javascript:void(0)" data-book-key="${book.key}">
      ...
      <fieldset legend="progress data">
        ...
        <label>stage: 
          <select name="stage">
            ${Array.from({length: stagesCountForTheBook}).map((_, i) => `<option ${i == stage ? 'selected' : ''} value="${i}">${i}</option>`).join('')}
          </select>
        </label><br>
      </fieldset>
      <div class="formErrors hidden"></div>
      <div class="successMessage hidden">Success</div>
  ...
  log() {
    ...
    const errorsEl = form.querySelector('.formErrors');
    const successEl = form.querySelector('.successMessage');
    const ranges = [{
      s : form.stage.value,
      f : form.from.value,
      t : form.to.value
    }];
    const res = BooksDomain.updateBookState(form.dataset.bookKey, ranges);
    if (res.result !== true) {
      errorsEl.innerHTML = `
        ${res.message}<br>
        ${res.details}
